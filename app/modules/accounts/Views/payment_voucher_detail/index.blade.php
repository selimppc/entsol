                        ...
                        @if(isset($model))
                            @foreach($model as $values)
                                <tr class="gradeX">
                                    ...
                                    <td>
                                        <a href="{{ route('view-payment-detail', $values->id) }}" class="btn btn-info btn-xs" data-toggle="modal" data-target="#etsbModal" data-placement="top" data-content="view"><i class="fa fa-eye"></i></a>
                                        @if($voucher_data->status !='posted')
                                            <a href="{{ route('edit-payment-detail', $values->id) }}" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#etsbModal" data-placement="top" data-content="edit"><i class="fa fa-edit"></i></a>
                                            <a href="{{ route('delete-payment-detail', $values->id) }}" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to Delete?')" data-placement="top" data-content="delete"><i class="fa fa-trash-o"></i></a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    @if(isset($voucher_data->status))
                        ...
                        @elseif($voucher_data->status == 'posted')
                            <h4 class="text-dark-green">
                                <strong>Payment Voucher({{$voucher_number}}) is Posted.</strong>
                            </h4>
                        @else
                            <h4 class="warning-report-text-color">
                                <strong>WARNING Report :</strong>
                                <span class="required"><strong>The Payment must balance ie. debits equal to credits before it can be processed.</strong></span>
                            </h4>
                        @endif
                    @endif
                </div>
    ...
    <div class="modal-dialog modal-lg search-z-index" style="width: 80%;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Payment Entry Detail # {{isset($voucher_number)?$voucher_number:''}}&nbsp;&nbsp;<span style="color: #A54A7B" class="user-guideline" data-content="<em>Must Fill <b>Required</b> Field.<b>*</b> Put cursor on input field for more informations</em>"><font size="2">(?)</font> </span></h4>
            </div>
            <div class="modal-body modal-backdrop">
                {!! Form::open(['route' => 'store-payment-detail','id' => 'jq-validation-form']) !!}
