...
<div class="form-group no-margin-hr panel-padding-h no-padding-t no-border-t">
    <div class="row">
        <div class="col-sm-6">
            ...
            {{--@if(count($role_id)>0)
                {!! Form::select('role_id', $role_id,Input::old('role_id'),['class' => 'form-control','required','title'=>'select  role, example :: Main Branch']) !!}
            @else
        ...
        <div class="row">
            <div class="col-sm-6">
                ...
                @if(count($permission_id)>0)
                    {!! Form::select('permission_id', $permission_id,Input::old('permission_id'),['class' => 'form-control','required','title'=>'select  permission, example :: Main Branch']) !!}
                @else
